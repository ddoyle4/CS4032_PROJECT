...
-- The name of the file under etcDirname in which to store
-- the token generated by the auth server
tokenFilename :: String
tokenFilename = "token"

-- The name of the file under etcDirname in which to store
-- the transaction ID generated by the transaction server
transIDFileName :: String
transIDFileName = "transaction_id"

-- The name of the file under etcDirname in which to store
-- the seed required to generate the client's encryption key, 
-- which is generated by the auth server
seedFilename :: String
...
performInitTransReq :: InitTransReq -> ConnectionInformation -> IO InitTransResp
performInitTransReq req info = do
  let str = "POST http://" ++ (hostAddr info) ++  ":" ++ (hostPort info) ++ "/initTransaction"
  let initReq = parseRequest_ str
  let request = setRequestBodyJSON req $ initReq
  response <- httpJSON request
  let ret = (getResponseBody response :: InitTransResp)
  return ret


performTransAbort :: AbortReq -> ConnectionInformation -> IO AbortResp
performTransAbort req info = do
  let str = "POST http://" ++ (hostAddr info) ++  ":" ++ (hostPort info) ++ "/abortTransaction"
  let initReq = parseRequest_ str
  let request = setRequestBodyJSON req $ initReq
  response <- httpJSON request
  let ret = (getResponseBody response :: AbortResp)
  return ret
  
performTransAction :: ActionReq -> ConnectionInformation -> IO ActionResp
performTransAction req info = do
  putStrLn $ "SENDING THIS: " ++ (show req) ++ " to " ++ (show info)
  let str = "POST http://" ++ (hostAddr info) ++  ":" ++ (hostPort info) ++ "/action"
  let initReq = parseRequest_ str
  let request = setRequestBodyJSON req $ initReq
  response <- httpJSON request
  let ret = (getResponseBody response :: ActionResp)
  return ret


performCommitTransReq :: CommitReq -> ConnectionInformation -> IO CommitResp
performCommitTransReq req info = do
  let str = "POST http://" ++ (hostAddr info) ++  ":" ++ (hostPort info) ++ "/commitTransaction"
  let initReq = parseRequest_ str
  let request = setRequestBodyJSON req $ initReq
  response <- httpJSON request
  let ret = (getResponseBody response :: CommitResp)
  return ret
...
removeETFFile :: String -> IO ()
removeETFFile name = do
  etcDirectory <- etcDir
  removeFile (etcDirectory ++ name)
...
writeTransID :: String -> IO ()
writeTransID id = do
  etcDirectory <- etcDir
  writeFile (etcDirectory ++ transIDFileName) (id)
  
readTransID :: IO String
readTransID = do
  etcDirectory <- etcDir
  rawData <- readFile (etcDirectory ++ transIDFileName)
  return rawData

transStart :: IO ()
transStart = do
  let initTransReq = InitTransReq
  cnxnInfo <- getConnectionInfo TransactionServer 
  resp <- performInitTransReq initTransReq cnxnInfo

  case (initReqStatus resp) of
    True -> do
      writeTransID (initTransID resp)
      putStrLn $ "Transaction started successfully with ID: " ++ (initTransID resp)
      return ()
      
    False -> do
      putStrLn $ "ERROR initialising request! Please check transaction server log"
      return ()

transCommit :: IO ()
transCommit = do
  id <- readTransID
  let commitReq = CommitReq id
  cnxnInfo <- getConnectionInfo TransactionServer 
  resp <- performCommitTransReq commitReq cnxnInfo
  removeETFFile transIDFileName
  putStrLn $ "Transaction " ++ id ++ " committed successfully" 
  return ()
...
transAbort = do
  ...
  let req   = AbortReq id
  cnxnInfo  <- getConnectionInfo TransactionServer 
  resp      <- performTransAbort req cnxnInfo
  case (abortStatus resp) of
    True -> do
      putStrLn $ "Transaction " ++ id ++  " aborted successfully"
      removeETFFile transIDFileName

    False -> putStrLn $ "Transaction " ++ id ++  " failed to abort!"

  return ()

transWrite :: [String] -> IO ()
transWrite params = do
  let path  = (params !! 0)
  token     <- getAuthToken
  key1      <- getEncryptionKeySeed
  id        <- readTransID
  ...
  let req   = ActionReq id "WRITE" path (encryptString fileData key1) token 
  resp      <- performTransAction req cnxnInfo
  putStrLn $ show resp
